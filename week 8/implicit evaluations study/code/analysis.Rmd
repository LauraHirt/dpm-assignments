...
```{r}
# overall AMP scores
dat_mean_amp_scores <- data_processed_after_exclusions %>%
  summarize(Mean = mean(amp_score, na.rm = TRUE),
            SD = sd(amp_score, na.rm = TRUE)) %>%
  mutate(group = "Full sample")


# AMP scores by gender category
dat_mean_amp_scores_by_gender <- data_processed_after_exclusions %>%
  group_by(group = gender) %>%
  summarize(Mean = mean(amp_score, na.rm = TRUE),
            SD = sd(amp_score, na.rm = TRUE))

# combine both into one sample
bind_rows(dat_mean_amp_scores,
          dat_mean_amp_scores_by_gender) %>%
